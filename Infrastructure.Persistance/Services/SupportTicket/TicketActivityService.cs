...
namespace Infrastructure.Persistance.Services.SupportTicket
{
    public class TicketActivityService : DABase, ITicketActivity
    {
        ...
        private const string SP_InsertTicketActivity = "InsertTicketActivity";
       
        private ILogger<TicketService> _logger;

        public TicketActivityService(IOptions<ConnectionSettings> connectionSettings, ILogger<TicketService> logger, IOptions<APISettings> settings) : base(connectionSettings.Value.DBCONN)
        {
            _logger = logger;
