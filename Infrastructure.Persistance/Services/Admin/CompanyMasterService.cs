...
namespace Infrastructure.Persistance.Services.Admin
{
    public class CompanyMasterService : DABase, ICompany
    {
        ...
        private const string SP_GetCompanyDetails = "GetCompanyDetails";

        private ILogger<CompanyMasterService> _logger;


        public CompanyMasterService(IOptions<ConnectionSettings> connectionSettings, ILogger<CompanyMasterService> logger, IOptions<APISettings> settings) : base(connectionSettings.Value.DBCONN)
        {
            _logger = logger;
        ...
        public async Task<CompanyList> GetCompany(CompanyMasterDTO companyMasterDTO)
        {
            ...
            try
            {
                ...
                {
                    response.Companies = await connection.QueryAsync<CompanyMasterDTO>(SP_GetCompanyDetails, new
                    {
